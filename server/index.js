...
// Generates expert insights from live card data for every race
// ---------------------------------------------------------------------------
app.get('/api/racing/intelligence', async (req, res) => {
  try {
    if (!racingSource || !process.env.RACING_API_KEY) {
...
function App_formatOddsFrac(dec) {
...
// ---------------------------------------------------------------------------
// RESULTS BY CONFIDENCE TIER (social proof leaderboard)
// ---------------------------------------------------------------------------
app.get('/api/results/by-confidence', (req, res) => {
  const results = readJSON('sample-results.json');
  const tips = readJSON('sample-tips.json');
...
function readNotifications() {
  try {
    return JSON.parse(fs.readFileSync(path.join(dataDir, 'notifications.json'), 'utf8'));
  } catch { return []; }
}

function writeNotifications(arr) {
  try {
    fs.writeFileSync(path.join(dataDir, 'notifications.json'), JSON.stringify(arr, null, 2));
  } catch (e) {
    console.error('[Notifications] Write failed:', e.message);
  }
}
...
function addNotification(entry) {
  try {
    ...
    writeNotifications(capped);
    return note;
  } catch (e) {
    console.error('[Notifications] Add failed:', e.message);
    return null;
  }
}
...
async function autoGenerateDailyTips() {
  ...
  existingTips.forEach(function(tip) {
    ...
    if (tip.date && tip.date < archiveCutoff && tip.status === 'active' && !tip.result) {
      tip.status = 'expired';
      tip.result = 'void';
      staleCleared++;
    }
  ...
  if (footballSource && process.env.API_FOOTBALL_KEY) {
    try {
      ...
      // Top leagues: PL(39), CL(2), EL(3), LaLiga(140), SerieA(135), Bundesliga(78), Ligue1(61),
      // Championship(40), EFL Cup(48), FA Cup(45), Eredivisie(88), Liga Portugal(94), Scottish Prem(179)
      ...
      // If no top-league fixtures today, use all fixtures as fallback
  ...
  // Select 2-4 tips with a MIX of price ranges (not all favourites)
  // Split racing candidates into price bands
  ...
  // Racing: try to pick one shorter-priced (< 5.0) and one value pick (5.0+)
  var racingShort = racingCandidates.filter(function(c) { return c.scored && c.scored.odds < 5; })
    .sort(function(a, b) { return b.edge - a.edge; });
  var racingValue = racingCandidates.filter(function(c) { return c.scored && c.scored.odds >= 5; })
    .sort(function(a, b) { return b.edge - a.edge; });

  var selectedRacing = [];
  if (racingShort.length > 0) selectedRacing.push(racingShort[0]);
  if (racingValue.length > 0) selectedRacing.push(racingValue[0]);
  // If we only got one band, fill from the other
  if (selectedRacing.length < 2 && racingCandidates.length >= 2) {
    racingCandidates.forEach(function(c) {
      if (selectedRacing.length < 2 && selectedRacing.indexOf(c) === -1) selectedRacing.push(c);
    });
  }
  selectedRacing = selectedRacing.slice(0, 2);

  var selectedFootball = footballCandidatesFinal.slice(0, 2);
  var selected = selectedRacing.concat(selectedFootball);

  // Sort by edge and cap at 4
...
async function autoSettleResults() {
  try {
    ...
    // Process ALL unsettled tips from last 3 days (includes voided ones that were never properly settled)
    ...
    var activeTips = tips.filter(function(t) {
      ...
      if (t.status === 'settled' && t.result && t.result !== 'void') return false; // already properly settled
      // Include: active tips, expired/voided tips that were never properly settled
        ...
        if (raceResults && raceResults.results) {
          activeTips.forEach(function(tip) {
            ...
            if (match) {
              ...
              updated++;
              console.log('[Auto-Settle] Racing: ' + tip.selection + ' = ' + tip.result + ' (' + pnl.toFixed(2) + 'u) [' + tip.date + ']');

              // Log notification for wins/placed
              try {
                if (tip.result === 'won' || tip.result === 'placed') {
                  addNotification({
                    type: 'tip_won',
                    message: 'Tip ' + (tip.result === 'won' ? 'won' : 'placed') + ': ' + tip.selection + ' @ ' + tip.odds + ' (+' + Math.abs(pnl).toFixed(2) + 'u)',
                    tipId: tip.id,
                    audience: tip.isPremium ? 'premium' : 'all',
                  });
                }
      ...
      try {
        ...
        activeTips.forEach(function(tip) {
          ...
          if (match) {
            ...
            updated++;
            console.log('[Auto-Settle] Football: ' + tip.selection + ' (' + match.homeTeam + ' ' + homeGoals + '-' + awayGoals + ' ' + match.awayTeam + ') = ' + tip.result + ' (' + pnl.toFixed(2) + 'u) [' + tip.date + ']');
            ...
            try {
              if (won) {
                addNotification({
                  type: 'tip_won',
                  message: 'Tip won: ' + tip.selection + ' @ ' + tip.odds + ' (+' + pnl.toFixed(2) + 'u)',
                  tipId: tip.id,
                  audience: tip.isPremium ? 'premium' : 'all',
                });
              }
...
async function scheduledDataRefresh() {
  ...
  try {
    ...
    try {
      ...
      // Re-read tips after settle may have updated them
      tips = readJSON('sample-tips.json');
      results = readJSON('sample-results.json');
    } catch (settleErr) {
      console.error('[Refresh] Pre-archive settle failed:', settleErr.message);
    }

    // 1. Archive unsettled tips that are 3+ days old (gives auto-settle time to work)
    var todayStr = uk.toISOString().split('T')[0];
    var archiveThreshold = new Date(uk.getTime() - 3 * 86400000).toISOString().split('T')[0];

    tips.forEach(function(tip) {
      if (tip.isWeeklyAcca) return;
      if (tip.date && tip.date < archiveThreshold && (tip.status === 'active' || (tip.status !== 'settled' && !tip.result))) {
        tip.status = 'expired';
        tip.result = 'void';
