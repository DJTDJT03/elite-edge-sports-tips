...
async function autoGenerateDailyTips() {
  ...
  // ===================================================================
  var MAX_MAIN_TIP_ODDS = 9.0;    // 8/1 maximum for main tips
  var MIN_OUTSIDER_ODDS = 7.0;    // 6/1 minimum for outsider
  var MAX_OUTSIDER_ODDS = 21.0;   // 20/1 maximum for outsider
  var MIN_MAIN_CONFIDENCE = 6;    // Main tips need conf 6+
  ...
  if (racingOutsider.length > 0) {
    ...
    // Don't duplicate — make sure it's not already in the main picks
  ...
  for (var i = 0; i < selected.length; i++) {
    if (!selected[i]._isOutsider && selected[i].confidence >= 7) { napIdx = i; break; }
  }
  if (napIdx === -1) {
    // Fallback: best non-outsider main tip
    for (var i = 0; i < selected.length; i++) {
      if (!selected[i]._isOutsider) { napIdx = i; break; }
    }
  }
