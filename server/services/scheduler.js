    ...
    try {
      ...
      if (hour >= 9 && hour <= 11) {
        ...
        if (validToday.length < 3) {
          if (hour === 9 || hour === 10) {
            console.log('[Failsafe] Only ' + validToday.length + ' valid tips at ' + hour + ':00 — force generating');
            await autoGenerateDailyTips(true);
          }
          if (hour === 11 && validToday.length < 3) {
            console.log('[Failsafe] ALERT — still only ' + validToday.length + ' tips at 11am');
            // Email admin
            try {
              var adminEmail = process.env.ADMIN_EMAIL || process.env.ADMIN_NOTIFICATION_EMAIL || 'darren@ecocleaningsystems.co.uk';
              emailService._sendEmail({
                to: adminEmail,
                subject: 'ALERT: Elite Edge — No tips generated for ' + today,
                html: '<h2 style="color:#ef4444;">Tip Generation Alert</h2><p>Only ' + validToday.length + ' valid tips exist for ' + today + ' at 11am. The system attempted auto-generation but failed. Please check the admin panel and use the "Generate Tips Now" button.</p><p><a href="https://eliteedgesports.co.uk/admin/#dashboard">Go to Admin</a></p>',
