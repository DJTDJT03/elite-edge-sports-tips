...
module.exports = function startScheduler(deps) {
  ...
  // =========================================================================
  // ---- Calibration auto-correct state (charter rule 6) ---------------------
  // Persisted in audit_log (action='calib_state'). `scale` multiplies published
  // confidence; it only moves when a confidence inversion is SUSTAINED for >=3
  // consecutive AutoTune nights, is bounded [0.90, 1.00], and self-heals (relaxes
  // back to 1.0) once calibration is healthy again. Bounded + reversible so it can
  // never runaway-suppress the engine.
  async function _loadCalibState() {
    try {
      if (db && db.query) {
        var r = await db.query("SELECT details FROM audit_log WHERE action='calib_state' ORDER BY timestamp DESC LIMIT 1");
        if (r.rows && r.rows[0] && r.rows[0].details) {
          var s = typeof r.rows[0].details === 'string' ? JSON.parse(r.rows[0].details) : r.rows[0].details;
          return { nights: s.nights || 0, scale: (s.scale > 0 ? s.scale : 1.0), lastDate: s.lastDate || null };
        }
      }
    } catch (e) { /* fall through to default */ }
    return { nights: 0, scale: 1.0 };
  }
  async function _saveCalibState(s) {
    try {
      if (db && db.query) {
        await db.query("INSERT INTO audit_log (user_id, user_email, action, entity, details, timestamp) VALUES ('system','system@eliteedge','calib_state','calibration',$1,NOW())", [JSON.stringify(s)]);
      }
    } catch (e) { /* non-fatal */ }
  }
  async function _getCalibScale() { var s = await _loadCalibState(); return s.scale > 0 ? s.scale : 1.0; }
  ...
  async function autoGenerateDailyTips(force) {
    ...
    if (footballSource && process.env.API_FOOTBALL_KEY) {
      try {
        ...
        // Calibration auto-correct: apply the persisted confidence scale (set by
        // AutoTune only after a sustained inversion; 1.0 when calibration is
        // healthy). Recalibrates published confidence before the gate, so when the
        // model has been over-confident we publish fewer, better-calibrated tips.
        try {
          var _calibScale = await _getCalibScale();
          if (_calibScale > 0 && _calibScale < 1.0) {
            for (var csi = 0; csi < allScoredFixtures.length; csi++) {
              var _cc = allScoredFixtures[csi];
              var _scaled = Math.max(1, Math.min(10, Math.round((_cc.confidence || 5) * _calibScale)));
              _cc.confidence = _scaled;
              if (_cc.scored) _cc.scored.confidence = _scaled;
            }
            console.log('[Auto-Tips] Applied calibration confidence scale ' + _calibScale);
          }
        } catch (e) { /* non-fatal — leave confidences as-is */ }
        ...
        if (cand.sport === 'football' && footballSource && process.env.API_FOOTBALL_KEY) {
          try {
            ...
            if (fmatch) {
              ...
              if (market.indexOf('result') !== -1 || market.indexOf('match') !== -1 || market.indexOf('win') !== -1) {
              ...
              } else if (selection.indexOf('over') !== -1 && /\d\.5/.test(selection)) {
                // Grade off the selection, not the market label ('Total Goals' has
                // no "over"/"under" substring — see the main settle path).
                ...
                else if (selection.indexOf('0.5') !== -1) won = totalGoals > 0;
              } else if (selection.indexOf('under') !== -1 && /\d\.5/.test(selection)) {
                if (selection.indexOf('3.5') !== -1) won = totalGoals < 4;
                else if (selection.indexOf('2.5') !== -1) won = totalGoals < 3;
                else if (selection.indexOf('1.5') !== -1) won = totalGoals < 2;
                else if (selection.indexOf('0.5') !== -1) won = totalGoals < 1;
              }
        ...
        try {
          ...
          for (var fti = 0; fti < activeTips.length; fti++) {
            ...
            if (fmatch) {
              ...
              // Over/Under — grade off the SELECTION, never the market label. The
              // consensus engine tags these 'Total Goals' (no "over"/"under"
              // substring), so the old market.indexOf('over') check silently failed
              // and graded EVERY Total Goals tip as a loss. The selection always
              // names the side + line (e.g. "Over 2.5 Goals"); the ".5" guard keeps
              // team names (e.g. an Asian Handicap "Team -1.5") out of this branch.
              else if (selection.indexOf('over') !== -1 && /\d\.5/.test(selection)) {
                if (selection.indexOf('3.5') !== -1) won = totalGoals > 3;
                else if (selection.indexOf('2.5') !== -1) won = totalGoals > 2;
                else if (selection.indexOf('1.5') !== -1) won = totalGoals > 1;
  ...
  async function scheduleDailyBulletin() {
    try {
      ...
      // Run any time after 8:00am UK — tips generate at 7:30am, email follows at 8am
      var isPastBulletinTime = hour >= 8;
  ...
  // FAILSAFE: Force generate if no valid tips by 9am, retry at 10am, alert admin at 11am
  setInterval(async function() {
    try {
      ...
      if (hour >= 9 && hour <= 11) {
        var tips = await db.getTips();
        var validToday = tips.filter(function(t) {
          var d = (t.date || '').toString().substring(0, 10);
          return d === today && t.id && t.id.toString().indexOf('auto_') === 0
            && t.selection && t.selection !== 'Unknown' && t.market && t.odds && t.odds > 0;
        });

        if (validToday.length === 0) {
          // Only retry if ZERO tips — 1+ tips is fine (fallback may produce fewer than 4)
          if (hour === 9) {
            console.log('[Failsafe] Zero tips at 9am — force generating');
            await autoGenerateDailyTips(true);
          }
        }
      }
    } catch(e) {
      console.error('[Failsafe] Error:', e.message);
    }
  ...
  setTimeout(function() {
    ...
    if (hour > 7 || (hour === 7 && minute >= 30)) {
      ...
      autoGenerateDailyTips().catch(function(e) { console.error('[Auto-Tips] Startup generate error:', e.message); });
    }
  }, 45000); // 45s delay — gives DB cleanup time to finish

  // Self-heal: the old grader keyed Over/Under off the MARKET label, so tips
  // tagged 'Total Goals' (no "over"/"under" substring) were graded as losses no
  // matter the score. Re-grade recently-settled goals results against the real
  // final score and flip any that are wrong. IDEMPOTENT — once corrected they
  // grade identically and nothing changes — so it's safe to run on every boot.
  async function regradeGoalsResults() {
    var sm = deps.sportMonks;
    if (!db || !db.getResults || !db.updateResult || !sm || !sm.isAvailable()) return;
    var all = await db.getResults();
    var cutoff = Date.now() - 60 * 86400000; // last 60 days
    var norm = function (s) { return String(s || '').toLowerCase().normalize('NFD').replace(/[^a-z0-9]/g, ''); };
    var targets = (all || []).filter(function (r) {
      if (!r || r.sport !== 'football') return false;
      if (r.result !== 'won' && r.result !== 'lost') return false;
      var sel = String(r.selection || '').toLowerCase();
      if (!/\d\.5/.test(sel) || (sel.indexOf('over') === -1 && sel.indexOf('under') === -1)) return false;
      var t = new Date(r.date).getTime();
      return isNaN(t) || t >= cutoff;
    });
    if (!targets.length) return;
    var byDate = {};
    targets.forEach(function (r) { var d = normDate(r.date); (byDate[d] = byDate[d] || []).push(r); });
    var corrected = 0, checked = 0;
    for (var date in byDate) {
      var fixtures = [];
      try { fixtures = await sm.getFixturesByDate(date) || []; } catch (e) { continue; }
      var finished = fixtures.filter(function (f) { return f && f.homeGoals != null && f.awayGoals != null; });
      if (!finished.length) continue;
      for (var i = 0; i < byDate[date].length; i++) {
        var r = byDate[date][i];
        var vs = String(r.event || '').split(/\s+vs\s+/i);
        if (vs.length < 2) continue;
        var home = norm(vs[0]), away = norm(vs[1].split(' - ')[0]);
        if (!home || !away) continue;
        var fx = finished.find(function (f) {
          var fh = norm(f.homeTeam), fa = norm(f.awayTeam);
          return fh && fa && (fh.indexOf(home) !== -1 || home.indexOf(fh) !== -1) && (fa.indexOf(away) !== -1 || away.indexOf(fa) !== -1);
        });
        if (!fx) continue;
        checked++;
        var total = (fx.homeGoals || 0) + (fx.awayGoals || 0);
        var sel = String(r.selection || '').toLowerCase();
        var line = sel.indexOf('3.5') !== -1 ? 3 : sel.indexOf('2.5') !== -1 ? 2 : sel.indexOf('1.5') !== -1 ? 1 : sel.indexOf('0.5') !== -1 ? 0 : null;
        if (line == null) continue;
        var won = sel.indexOf('over') !== -1 ? total > line : total < (line + 1);
        var correct = won ? 'won' : 'lost';
        if (correct !== r.result) {
          var stake = r.stake || 2, odds = r.odds || 2;
          var pnl = won ? Math.round((odds - 1) * stake * 100) / 100 : -stake;
          try {
            await db.updateResult(r.id, { result: correct, pnl: pnl });
            corrected++;
            console.log('[RegradeGoals] Fixed "' + r.event + '" ' + r.selection + ' | score ' + fx.homeGoals + '-' + fx.awayGoals + ' → ' + correct + ' (was ' + r.result + ')');
          } catch (e) { /* skip */ }
        }
      }
    }
    if (corrected) console.log('[RegradeGoals] Corrected ' + corrected + ' mis-graded goals result(s); ' + checked + ' checked.');
  }
  setTimeout(safeRun('RegradeGoals', regradeGoalsResults), 120000); // 2 min after boot, once settle has warmed the fixture cache
  ...
  async function telegramDailyContent() {
    ...
    // Morning teaser at 8:15am (after bulletin email at 8am)
    if (hour === 8 && minute >= 15 && minute <= 30 && lastTgMorning !== dateStr) {
  ...
  async function autoTuneAnalysts(force) {
    ...
    try {
      ...
      if (calibration.ready) {
        // AUTO-CORRECT (charter rule 6): act only on a SUSTAINED inversion — a
        // higher-confidence band winning LESS than a lower band — held for >=3
        // consecutive nights. Nudge the published-confidence scale down a notch
        // (bounded 0.90) with receipts; reset the streak after acting. When
        // healthy, relax the scale back toward 1.0 and clear the streak.
        var _cst = await _loadCalibState();
        var _nights = _cst.nights || 0, _scale = _cst.scale > 0 ? _cst.scale : 1.0, _acted = null;
        // Idempotent per calendar day — a manual force-run can't double-count a night.
        if (_cst.lastDate === dateStr) {
          console.log('[AutoTune] Calibration streak already updated today (' + dateStr + ') — scale=' + _scale);
          calibration.autoAction = { scale: _scale, consecutiveInversionNights: _nights, acted: null };
        } else {
          // Strict calendar-consecutiveness: if the last update wasn't yesterday,
          // a night was missed (deploy/outage) → the streak is broken, reset it.
          var _yday = new Date(uk.getTime() - 86400000).toISOString().split('T')[0];
          if (_cst.lastDate && _cst.lastDate !== _yday) _nights = 0;
          if (!calibration.confidenceOrdering.healthy) {
            _nights += 1;
            if (_nights >= 3) {
              var _newScale = Math.max(0.90, Math.round((_scale - 0.05) * 100) / 100);
              _acted = { from: _scale, to: _newScale, afterNights: _nights, receipts: calibration.confidenceOrdering.inversions };
              _scale = _newScale; _nights = 0; // acted → reset the streak
              console.log('[AutoTune] Calibration AUTO-CORRECT: sustained inversion → confidence scale ' + _acted.from + ' → ' + _acted.to + ' (receipts: ' + _acted.receipts.join('; ') + ')');
            } else {
              console.log('[AutoTune] Calibration inversion night ' + _nights + '/3 (no action yet): ' + calibration.confidenceOrdering.inversions.join('; '));
            }
          } else {
            _nights = 0;
            if (_scale < 1.0) { _scale = Math.min(1.0, Math.round((_scale + 0.05) * 100) / 100); console.log('[AutoTune] Calibration healthy → relaxing confidence scale to ' + _scale); }
          }
          await _saveCalibState({ nights: _nights, scale: _scale, lastDate: dateStr, updatedAt: new Date().toISOString(), lastInversions: calibration.confidenceOrdering.inversions });
          calibration.autoAction = { scale: _scale, consecutiveInversionNights: _nights, acted: _acted };
        }
        console.log('[AutoTune] Calibration (30d): grade=' + calibration.grade + ', Brier=' + calibration.brier + ', ordering=' + (calibration.confidenceOrdering.healthy ? 'healthy' : 'INVERSIONS: ' + calibration.confidenceOrdering.inversions.join('; ')) + ' | confidence scale=' + _scale);
      } else {
  ...
  return { autoSettleResults, autoGenerateDailyTips, checkTrialExpiries, checkOddsMovementAlerts, dailyHealthCheck, autoTuneAnalysts, updateWeeklyBlog };
};
